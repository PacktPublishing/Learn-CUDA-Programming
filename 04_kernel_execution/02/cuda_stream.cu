                          ...
                          const int size, const int bufsize, cudaStream_t stream = 0)
{
    ...
    printf("3) copy device -> host\n");

    // transfer the result
    cudaMemcpyAsync(h_c, d_c, bufsize, cudaMemcpyDeviceToHost, stream);

    printf("4) device sync\n");

    // synchronization with stream instead of cudaDeviceSynchronize()
