#include <iostream>
#include <string>
#include "opencv2/opencv.hpp"

void BlurHost(std::string filename, float scale)
{
    cv::Mat src = cv::imread(filename, 1);
    cv::Mat dst;

    cv::TickMeter tm;

    tm.reset();
    tm.start();
    //cv::resize(src, dst, cv::Size(), scale, scale);
    cv::bilateralFilter(src, dst, 10, 50, 50);
    tm.stop();

    std::cout << "CPU Time: " << tm.getTimeMilli() << " ms." << std::endl;

    cv::imwrite("result_host.jpg", dst);
}

void BlurCuda_Stream(std::string filename, float scale)
{
    cv::Mat::setDefaultAllocator(cv::cuda::HostMem::getAllocator(cv::cuda::HostMem::PAGE_LOCKED));
    
    cv::Mat src = cv::imread(filename, 1);
    cv::Mat dst = cv::Mat(src.rows * scale, src.cols * scale, CV_8UC1);
    //cv::cuda::GpuMat dst_cuda, src_cuda;
    cv::cuda::GpuMat src_cuda = cv::cuda::GpuMat(src.rows, src.cols, CV_8UC1);
    cv::cuda::GpuMat dst_cuda = cv::cuda::GpuMat(src.rows * scale, src.cols * scale, CV_8UC1);

    cv::TickMeter tm;

    // create Stream object
    cv::cuda::Stream stream;

    // warps-up
    cv::cuda::bilateralFilter(src_cuda, dst_cuda, 10, 50, 50);

    tm.reset();
    tm.start();
    src_cuda.upload(src, stream);
    cv::cuda::bilateralFilter(src_cuda, dst_cuda, 10, 50, 50);
    dst_cuda.download(dst, stream);
    stream.waitForCompletion();
    tm.stop();

    std::cout << "GPU Time: " << tm.getTimeMilli() << " ms." << std::endl;
    cv::imwrite("result_cuda.jpg", dst);
}

int main(int argc, char *argv[])
{
    std::string filename("flower.jpg");
    float scale = 1.5f;

    ResizeHost(filename, scale);
    BlurCuda_Stream(filename, scale);

    return 0;
}